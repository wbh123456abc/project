...
class Game:
    def __init__(self):
        ...
        self.env = football_env.create_environment(
                ...
                write_goal_dumps=False, write_full_episode_dumps=False, render=False, rewards='scoring', 
                representation='simple115')
        ob = self.env.reset()
        self.stobs = self._process_obs(ob)
...
def main():
    ...
    for i in range(1000):
        ...
        while done == 0:
            ...
            if info:
                print("%d Steps: %d Reward: %.2f" % (i, info['length'], info['reward']))
                if info['reward'] == 1.0:
                    tot += 1
